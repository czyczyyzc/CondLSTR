...
class ParquetSampler(Sampler[List[int]]):
    ...
    def __init__(self, data_source: Sized, batch_size: int, shuffle: bool = False, drop_last: bool = False,
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        if self.shuffle:
            # shuffle files
            index = torch.randperm(len(data_index_list), generator=generator).tolist()
            data_index_list = [data_index_list[i] for i in index]
            ...
            for i, data_index in enumerate(data_index_list):
                ...
                data_index_list[i] = data_index[index]

        batch_list = []
        for i, data_index in enumerate(data_index_list):
            num_batches = len(data_index) // self.batch_size
            batches = data_index[: num_batches * self.batch_size].reshape(num_batches, self.batch_size).tolist()
            if not self.drop_last and (len(data_index) > num_batches * self.batch_size):
                batches.append(data_index[num_batches * self.batch_size:].tolist())
    ...
    def __len__(self) -> int:
        ...
        if self.drop_last:
            return sum([len(index) // self.batch_size for index in self.data_index_list])
        else:
            return sum([(len(index) + self.batch_size - 1) // self.batch_size for index in self.data_index_list])


class DistributedParquetSampler(Sampler[T_co]):
    ...
    def __init__(self, dataset: Dataset, batch_size: int, num_replicas: Optional[int] = None,
        ...
        if self.drop_last:
            num_batches = sum([len(index) // self.batch_size for index in self.data_index_list])
            ...
            num_batches = sum([(len(index) + self.batch_size - 1) // self.batch_size for index in self.data_index_list])
    ...
    def __iter__(self) -> Iterator[T_co]:
        ...
        if self.shuffle:
            # shuffle files
            index = torch.randperm(len(data_index_list), generator=g).tolist()
            data_index_list = [data_index_list[i] for i in index]
            # shuffle data in each file
            for i, data_index in enumerate(data_index_list):
