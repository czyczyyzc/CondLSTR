    ...
    def __init__(self, in_planes, out_planes, block_num=3, stride=1, norm_layer=nn.BatchNorm2d):
        ...
        for idx in range(block_num):
            if idx == 0:
                ...
                self.conv_list.append(ConvX(out_planes // int(math.pow(2, idx)), out_planes // int(math.pow(2, idx)), norm_layer=norm_layer))
...
class STDCNet1446(nn.Module):
    def __init__(self, base=64, layers=[4, 5, 3], block_num=4, type="cat", num_classes=1000, dropout=0.20,
                 pretrain_model='', use_conv_last=False, norm_layer=nn.BatchNorm2d, use_checkpoint=False):
        ...
        self.features = self._make_layers(base, layers, block_num, block, norm_layer=norm_layer)
        if use_conv_last:
            self.conv_last = ConvX(base * 16, max(1024, base * 16), 1, 1, norm_layer=norm_layer)
    ...
    def forward(self, x):
        feat2, feat4, feat8, feat16, feat32 = None, None, None, None, None
