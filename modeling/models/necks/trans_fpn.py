...
class TransConvEncoderModule(nn.Module):
    def __init__(self, in_dim, attn_in_dims, attn_out_dims, strides, ratios, downscale=True, with_pos=True):
        ...
        self.pos_shape = with_pos
        self.pos_embeds = nn.ModuleList()
        if with_pos is not None:
            for dim in attn_out_dims:
                self.pos_embeds.append(PositionEmbeddingSine(dim // 2, normalize=True))
    ...
    def forward(self, src):
        ...
        if self.pos_shape is None:
            ...
            for layer, pos in zip(self.attn_layers, self.pos_embeds):
                b, c, h, w = src.shape
                msk = torch.zeros((b, h, w), dtype=torch.bool, device=src.device)
                src = layer(src, pos(msk))
