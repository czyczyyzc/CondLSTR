...
class LaneEval(object):
    ...
    def bench_one_submit(self, pred_file, gt_file, prob_th=0.5, vis=False):
        ...
        x_error_close_avg = np.nanmean(laneline_x_error_close)
        x_error_far_avg = np.nanmean(laneline_x_error_far)
        z_error_close_avg = np.nanmean(laneline_z_error_close)
        z_error_far_avg = np.nanmean(laneline_z_error_far)
